...
var $description2ID = 0;
var $_translations = null;
var $_currentPage = PAGE_BDD_ENTRY_DESCRIPTIONS_2;
var $_lastPageSize = 0;

function actionFormatter( value, row, index ){
    $pageOffset = ($('#Descriptions2ListTable').bootstrapTable('getOptions')['pageNumber']-1) * $('#Descriptions2ListTable').bootstrapTable('getOptions')['pageSize']+index;
    $sortASC = ($('#Descriptions2ListTable').bootstrapTable('getOptions')['sortOrder']=='asc')?'1':'0';
    $rightLink = window.ARCHIVIST_JSON_URLS.bs_idp_archivist_managedb_input_descriptions2_finetune;
    $rightLink += '?description2Id=' + row['id'];
...
function onClickBtnSuppressModalConfirm(){
	$dataStr = "id=" + $description2ID;
	$url = window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_descriptions2_delete;
	$.ajax({
		type: "GET",
		url: $url,
		data: $dataStr,
		cache: false,
		success: function($response) {
			$('#Descriptions2ListTable').bootstrapTable('refresh');
		},
		error: function (xhr, ajaxOptions, thrownError) {
...
function onClickBtnModifyModalConfirm(){
	$dataStr = "id=" + $description2ID + "&name=" + encodeURIComponent($('#frm_modify_name').val());
	$url = window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_descriptions2_modify;
	$.ajax({
		type: "GET",
		url: $url,
		data: $dataStr,
...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        success: function($response) {
            ...
            initBDDTab( $('#Descriptions2ListTable'), $response.data, $_pageOffset, $_sortASC );
        },
        error: function (xhr, ajaxOptions, thrownError) {
    ...
    $('#btn_add').click( function(){
        if( $('#frm_name').val()==null || $('#frm_name').val().trim().length <= 0 ){
            popError( $('#div_frm_name'), "Le nom du descriptif 2 ne peut pas être vide.", 'top' );
            return false;
        } else {
            $dataStr = "name=" + encodeURIComponent($('#frm_name').val());
            $pageNumber = $('#Descriptions2ListTable').bootstrapTable('getOptions')['pageNumber'];
            $url = window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_descriptions2_add;
            $.ajax({
                type: "GET",
                url: $url,
                data: $dataStr,
                ...
                success: function ($response) {
                    $('#frm_name').val('');
                    $('#Descriptions2ListTable').bootstrapTable('refresh', {'pageNumber': $pageNumber});
                },
                error: function (xhr, ajaxOptions, thrownError) {
                    $('#frm_name').val('');
                    alert($_translations[15]);
