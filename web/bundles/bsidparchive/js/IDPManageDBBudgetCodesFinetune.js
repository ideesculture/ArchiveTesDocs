var budgetcode_id = window.IDP_DATA.idp_budgetcode_id;
var all_services = [];
...
function setLinkSelected( row ){
	...
	$.ajax({
		type: "GET",
		url: window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_budgetcodes_links_set,
		data: { budgetCodeId: budgetcode_id, serviceId: row['id'] },
		cache: false,
...
function unsetLinkSelected( row ){
	...
	$.ajax({
		type: "GET",
		url: window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_budgetcodes_links_unset,
		data: { budgetCodeId: budgetcode_id, serviceId: row['id'] },
		cache: false,
		success: function( data, status ){
			$("#waitAjax").hide();
...
function setAllLinkSelected( ){
	...
	$.ajax({
		type: "GET",
		url: window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_budgetcodes_links_set,
		data: { budgetCodeId: budgetcode_id, serviceId: -1 },
		cache: false,
		success: function( data, status ){
			$("#waitAjax").hide();
		},
...
function loadBudgetCodeLink( $data ){
	...
	$.ajax({
		type: "GET",
		url: window.ARCHIVIST_JSON_URLS.bs_idp_archivist_json_budgetcodes_links_list,
		data: { budgetCodeId: budgetcode_id },
		cache: false,
