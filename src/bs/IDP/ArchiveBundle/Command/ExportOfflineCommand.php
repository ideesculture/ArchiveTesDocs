...
class ExportOfflineCommand extends ContainerAwareCommand
{
    // For test purpose, command for all archives in Consulter
    // php bin/console app:export-table-offline 1 0 [] "[name,ordernumber,service]" 0 "[;;;;;;;;;;;;;1900,2199;1900,2199;;;;;2;;5;0;0;;;;;;;]" 2 debug

    // For test purpose, command for admin export all
    // php bin/console app:export-table-offline 1 1 0 0 0 0 0 debug
    ...
    {
        $this
            ->setName('app:export-table-offline')
            ->setDescription('Export table offline, and save it in the user space')
            ->addArgument('userId', InputArgument::REQUIRED, 'user who ask for print')
            ->addArgument('all', InputArgument::REQUIRED, 'export all (0) or export table (1)')
            ->addArgument('listId', InputArgument::REQUIRED, 'list of id to print out, empty/null for all table')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if( $debugMode ){
            ...
            $output->writeln( '<info>Archimage Export Offline Process</info>' );
            $output->writeln( '<info>--------------------------------</info>' );
            $output->writeln( '' );
        }
        ...
        if( $whereAmI >= 7 )
            $whereAmI = $this->preciseWhereAmI( $whereAmI, $xpsearch );
        $fctCall = $this->computeCallFromWhereAmI( $whereAmI );
        ...
        $name = "Export Total";
        $partialFilename = "_ExportAll";
        if( $all != 1 ) {
            $name = "Export du tableau page " . IDPConstants::$PRINT_OFFLINE_READABLE_NAME[$whereAmI];
            $partialFilename = "_ExportTable_" . $whereAmI;
        }
        $userFile->setName( $name );
        $userFile->setFilename( $userId. $partialFilename . '_' . time() . '.xlsx' );
        $userFile->setFiletype( IDPUserFiles::FILETYPE_XLS );
        ...
        if( $all == 1 ){ // Export ALL
            $fileSize = IDPExportTableCommon::makeExportFile(
                $doctrine,
                null, null, null, null, null, null, null,
                null, null, null, null, IDPGlobalSettings::EXPORT_TYPE_XLS,
                IDPExportTableCommon::EXPORT_FILE, $userFile, $debugMode?$output:null
            );
        } else { // Export not all, only current table
            $userServices = $doctrine->getRepository('bsCoreUsersBundle:IDPUserServices')->findBy(
                array('user' => $userId ), null );
            $search = $this->translateXPSearch( $xpsearch );

            $fileSize = IDPExportTableCommon::makeExportFile(
                $doctrine,
                $listId, $userServices, $fctCall, $search, $search['xpwhat'], $search['xpwhere'], $search['xphow'],
                $search['xpwith'], $search['xpstate'], $search['filterprovider'], $listColumn, IDPGlobalSettings::EXPORT_TYPE_XLS,
                IDPExportTableCommon::EXPORT_FILE, $userFile, $debugMode?$output:null );
        }

        if( $debugMode )
            $output->writeln( 'End of process, close progress' );
