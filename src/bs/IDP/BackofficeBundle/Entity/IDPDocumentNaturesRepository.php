...
/**
 * IDPDocumentNaturesRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class IDPDocumentNaturesRepository extends EntityRepository
{
	...
	{
		$querySelect = "SELECT d FROM bsIDPBackofficeBundle:IDPDocumentNatures d";
    ...
    public function getAllIndexedOnID( ){
        $result = [];
        $documentNatures = $this->getAll();
        if( !$documentNatures ) return null;

        foreach( $documentNatures as $documentNature ){
            $result[$documentNature['id']] = $documentNature;
        }
        return $result;
    }

    public function countDocumentNatures( $search ){

		$queryStr = 'SELECT COUNT(d.id) ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDocumentNatures d ';
        if( $search )
	...
	public function loadDocumentNaturesDatas( $search, $sort, $order, $limit, $offset ){
		$queryStr = 'SELECT d ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDocumentNatures d ';
        if( $search )
            $queryStr .= " WHERE d.longname LIKE '%$search%' ";
	...
	public function findOneWithConstraint( $longname, $service_id ){
        ...
        $query = $this->createQueryBuilder( 'dn' )
            ->select( 'dn' )
            ->leftJoin( 'dn.services', 's' )
            ->addSelect( 's' )
            ->where( 'dn.longname = :longname' )
            ->andWhere( 's.id = :sid' )
    ...
    public function findPrevNext( $documentNaturesId, $sortASC ){
        $em = $this->getEntityManager();

        // On récupère le nom de l'élément courant
        $query = $em->createQuery( "SELECT t.longname FROM bsIDPBackofficeBundle:IDPDocumentNatures t WHERE t.id = :currentid" )
            ->setParameter( 'currentid', $documentNaturesId );
        ...
        $query = $em->createQuery( "SELECT MIN(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDocumentNatures t WHERE t.longname > :currentname ORDER by t.longname" )
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDocumentNatures t WHERE t.longname = :nextname" )
        ...
        $query = $em->createQuery( "SELECT MAX(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDocumentNatures t WHERE t.longname < :currentname ORDER by t.longname" )
            ->setParameter( 'currentname' , $currentName )
            ->setMaxResults( 1 );
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDocumentNatures t WHERE t.longname = :previousname" )
        ...
        if( $sortASC == 1 )
            return [
                ...
                'current' => ['id' => $documentNaturesId, 'longname' => $currentName ],
