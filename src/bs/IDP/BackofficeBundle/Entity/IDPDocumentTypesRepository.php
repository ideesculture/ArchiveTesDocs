...
/**
 * IDPDocumentTypesRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class IDPDocumentTypesRepository extends EntityRepository
{
	...
	{
		$querySelect = "SELECT d FROM bsIDPBackofficeBundle:IDPDocumentTypes d";
    ...
    public function getAllIndexedOnID( ){
        ...
        foreach( $documentTypes as $documentType ){
            $result[$documentType['id']] = $documentType;
        }
        return $result;
    }

    public function countDocumentTypes( $search ){

		$queryStr = 'SELECT COUNT(d.id) ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDocumentTypes d ';
        if( $search )
	...
	public function loadDocumentTypesDatas( $search, $sort, $order, $limit, $offset ){
		$queryStr = 'SELECT d ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDocumentTypes d ';
        if( $search )
            $queryStr .= " WHERE d.longname LIKE '%$search%' ";
	...
	public function findOneWithConstraint( $longname, $documentnature_id ){

        $query = $this->createQueryBuilder( 'dt' )
            ->select( 'dt' )
            ->leftJoin( 'dt.documentNatures', 'dn' )
            ->addSelect( 'dn' )
            ->where( 'dt.longname = :longname' )
            ->andWhere( 'dn.id = :dnid' )
            ->setParameter( 'longname', $longname )
            ->setParameter( 'dnid', $documentnature_id );

        return $query->getQuery()->getResult();

	}

    public function findPrevNext( $documentTypeId, $sortASC ){
        $em = $this->getEntityManager();

        // On récupère le nom de l'élément courant
        $query = $em->createQuery( "SELECT t.longname FROM bsIDPBackofficeBundle:IDPDocumentTypes t WHERE t.id = :currentid" )
            ->setParameter( 'currentid', $documentTypeId );
        ...
        $query = $em->createQuery( "SELECT MIN(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDocumentTypes t WHERE t.longname > :currentname ORDER by t.longname" )
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDocumentTypes t WHERE t.longname = :nextname" )
        ...
        $query = $em->createQuery( "SELECT MAX(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDocumentTypes t WHERE t.longname < :currentname ORDER by t.longname" )
            ->setParameter( 'currentname' , $currentName )
            ->setMaxResults( 1 );
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDocumentTypes t WHERE t.longname = :previousname" )
        ...
        if( $sortASC == 1 )
            return [
                ...
                'current' => ['id' => $documentTypeId, 'longname' => $currentName ],
