...
/**
 * IDPDescriptions2Repository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class IDPDescriptions2Repository extends EntityRepository
{
	...
	{
		$querySelect = "SELECT d FROM bsIDPBackofficeBundle:IDPDescriptions2 d";
    ...
    public function countDescriptions2( $search ){

		$queryStr = 'SELECT COUNT(d.id) ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDescriptions2 d ';
        if( $search )
	...
	public function loadDescriptions2Datas( $search, $sort, $order, $limit, $offset ){
		$queryStr = 'SELECT d ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPDescriptions2 d ';
        if( $search )
            $queryStr .= " WHERE d.longname LIKE '%$search%' ";
    ...
    public function findPrevNext( $description2Id, $sortASC ){
        $em = $this->getEntityManager();

        // On récupère le nom de l'élément courant
        $query = $em->createQuery( "SELECT t.longname FROM bsIDPBackofficeBundle:IDPDescriptions2 t WHERE t.id = :currentid" )
            ->setParameter( 'currentid', $description2Id );
        ...
        $query = $em->createQuery( "SELECT MIN(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDescriptions2 t WHERE t.longname > :currentname ORDER by t.longname" )
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDescriptions2 t WHERE t.longname = :nextname" )
        ...
        $query = $em->createQuery( "SELECT MAX(t.longname) as longname FROM bsIDPBackofficeBundle:IDPDescriptions2 t WHERE t.longname < :currentname ORDER by t.longname" )
            ->setParameter( 'currentname' , $currentName )
            ->setMaxResults( 1 );
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPDescriptions2 t WHERE t.longname = :previousname" )
        ...
        if( $sortASC == 1 )
            return [
                ...
                'current' => ['id' => $description2Id, 'longname' => $currentName ],
