...
/**
 * IDPBudgetCodesRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class IDPBudgetCodesRepository extends EntityRepository
{
	...
	{
		$querySelect = "SELECT b FROM bsIDPBackofficeBundle:IDPBudgetCodes b";
    ...
    public function getAllIndexedOnID( ){
        $result = [];
        $budgetCodes = $this->getAll();
        if( !$budgetCodes ) return null;

        foreach( $budgetCodes as $budgetCode ){
            $result[$budgetCode['id']] = $budgetCode;
        }
        return $result;
    }

    public function countBudgetCodes( $search ){

		$queryStr = 'SELECT COUNT(b.id) ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPBudgetCodes b ';
        if( $search )
            $queryStr .= " WHERE b.longname LIKE '%$search%' ";
	...
	public function loadBudgetCodesDatas( $search, $sort, $order, $limit, $offset ){
		$queryStr = 'SELECT b ';
		$queryStr .= ' FROM bsIDPBackofficeBundle:IDPBudgetCodes b ';
        if( $search )
            $queryStr .= " WHERE b.longname LIKE '%$search%' ";
	...
	public function findOneWithConstraint( $longname, $service_id ){
        ...
        $query = $this->createQueryBuilder( 'bc' )
            ->select( 'bc' )
            ->leftJoin( 'bc.services', 's' )
            ->addSelect( 's' )
            ->where( 'bc.longname = :longname' )
            ->andWhere( 's.id = :sid' )
    ...
    public function findPrevNext( $budgetCodeId, $sortASC ){
        $em = $this->getEntityManager();

        // On récupère le nom de l'élément courant
        $query = $em->createQuery( "SELECT t.longname FROM bsIDPBackofficeBundle:IDPBudgetCodes t WHERE t.id = :currentid" )
            ->setParameter( 'currentid', $budgetCodeId );
        ...
        $query = $em->createQuery( "SELECT MIN(t.longname) as longname FROM bsIDPBackofficeBundle:IDPBudgetCodes t WHERE t.longname > :currentname ORDER by t.longname" )
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPBudgetCodes t WHERE t.longname = :nextname" )
        ...
        $query = $em->createQuery( "SELECT MAX(t.longname) as longname FROM bsIDPBackofficeBundle:IDPBudgetCodes t WHERE t.longname < :currentname ORDER by t.longname" )
            ->setParameter( 'currentname' , $currentName )
            ->setMaxResults( 1 );
        ...
        $query = $em->createQuery( "SELECT t.id FROM bsIDPBackofficeBundle:IDPBudgetCodes t WHERE t.longname = :previousname" )
        ...
        if( $sortASC == 1 )
            return [
                ...
                'current' => ['id' => $budgetCodeId, 'longname' => $currentName ],
